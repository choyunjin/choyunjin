# Insertion effect
Insertion effect는 지정된 채널에 갖가지 이펙트(distortion,overdrive 등등등 뭐가 많음)를 넣을 수 있는 기능으로, lrsynth에서는 1포트당 8개까지 사용 가능하며, 1번째 이펙터는 Roland GS sysex 로도 제어할 수 있음

# Sysex

## 기본 구조
```
f0 00 34 0f 10 0b 15 rr rr rr vv ... f7
```
- `f0` = 이 메세지가 sysex 메세지임을 의미함
- `00 34 0f` = 생산자(manufacturer) ID (YJ)
- `10` = Device ID (사실상 무쓸모지만 가급적 10으로 놔두는 것을 권장)
...
## 이펙트 종류 설정
```
f0 00 34 0f 10 0b 15 20 1x 00 vv vv vv f7
```
- `f0 00 34 0f 10 0b 15` = (생략)
...
## 채널별 이펙트 on/off
```
f0 00 34 0f 10 0b 15 20 2x 0h vv f7
```
- `f0 00 34 0f 10 0b 15` = (생략)
...
## 이펙트 파라미터
```
f0 00 34 0f 10 0b 15 20 3x pp vv f7
```
- `f0 00 34 0f 10 0b 15` = (생략)
...
### 0x27~0x2f 파라미터에 대해
0x27~0x2f 파라미터는 이펙터의 종류에 상관없이 공통으로 들어가는 파라미터이다.
이 부분의 경우 가장 마지막으로 적용되는 이펙트 쪽의 설정을 적용한다.

| 파라미터 번호 | 이름 | 값 | 기본값 | 설명 |
| --- | -------- | ----- | --- | ------------ |
| 0x27 | Reverb send level | 0 - 127<br>(0x00 - 0x7f) | 127 (0x7f) | 이펙트가 적용되는 채널의 Reverb send level을 조정한다. |
| 0x28 | Chorus send level | 0 - 127<br>(0x00 - 0x7f) | 127 (0x7f) | 이펙트가 적용되는 채널의 Chorus send level을 조정한다. |
| 0x29 | Delay send level | 0 - 127<br>(0x00 - 0x7f) | 127 (0x7f) | 이펙트가 적용되는 채널의 Delay send level을 조정한다. |
| 0x2a | Ignore CC parameters | off,on<br>(0x00 - 0x01) | off (0x00) | on로 설정 시 각 채널의 control change 설정값을 사용하지 않고 위 3개 값을 사용하도록 강제한다. |
| 0x2f | EQ switch | off,on<br>(0x00 - 0x01) | on (0x01) | GS sysex로 정하는 EQ를 여기에도 적용할지 설정한다. |
...
| 파라미터 번호 | 이름 | 값 | 기본값 | 설명 |
| --- | -------- | ----- | --- | ------------ |
| 0x01 | Low freq | 200Hz,400Hz(N/A) | N/A | 저음역대의 기준 주파수를 설정한다. |
| 0x02 | Low gain | -12 - +12<br>(0x34 - 0x40 - 0x4c) | 0 (0x40) | 저음역대의 gain을 조정한다. |
| 0x03 | High freq | 4kHz,8kHz(N/A) | N/A | 고음역대의 기준 주파수를 설정한다. |
| 0x04 | High gain | -12 - +12<br>(0x34 - 0x40 - 0x4c) | 0 (0x40) | 고음역대의 gain을 조정한다. |
| 0x05 | Mid1 freq | 200 - 6300(0x00 - 0x78)[^EQFreq] | N/A | 중간 음역대 1의 기준 주파수를 설정한다. |
| 0x07 | Mid1 gain | -12 - +12<br>(0x34 - 0x40 - 0x4c) | 0 (0x40) | 중간 음역대 1의 gain을 조정한다. |
| 0x08 | Mid2 freq | 200 - 6300(0x00 - 0x78)[^EQFreq] | N/A | 중간 음역대 2의 기준 주파수를 설정한다. |
| 0x0a | Mid2 gain | -12 - +12<br>(0x34 - 0x40 - 0x4c) | 0 (0x40) | 중간 음역대 2의 gain을 조정한다. |
...
| 0x02 | Sustain | 0 - 127<br>(0x00 - 0x7f) | N/A | 사실상 input gain이다. |
...
### 값 계산 공식 각주
몇몇 개는 사실상 공식도 아님
[^Rate1]: Rate1(Hz) 공식: x <= 0x63 일 때 0.05*x, 0x64 <= x <= 0x77 일 때 5+(0.1(x - 99)), 0x78 <= x <= 0x7d 일 때 7+(0.5(x - 119)), x >= 0x7e 일 때 10
[^HFDamp]: HFDamp(Hz) 공식(사실 공식도 아님): a = floor(x / 8) 이라 하자. 이때 a값에 따라 0부터 순서대로 { 315,400,500,630,800,1000,1250,1600,2000,2500,3150,4000,5000,6300,8000,8000 }
[^CutoffFreqLPF]: CutoffFreq,LPF(Hz) 공식(사실 공식도 아님): a = floor(x / 8) 이라 하자. 이때 a값에 따라 0부터 순서대로 { 250,315,400,500,630,800,1000,1250,1600,2000,2500,3150,4000,5000,6300,6300 }
[^EQFreq]: EQFreq(Hz) 공식(사실 공식도 아님): a = floor(x / 8) 이라 하자. 이때 a값에 따라 0부터 순서대로 { 200,250,315,400,500,630,800,1000,1250,1600,2000,2500,3150,4000,5000,6300 }
